...
async function getExhibitions() {
  try {
    const res = await fetch(`${BASE_URL}/exhibition/all`, {
      next: {
        tags: ["exhibition"],
      },
    });
...
async function getPhotos() {
  try {
    const res = await fetch(`${BASE_URL}/photo/all`, {
      next: {
        tags: ["photos"],
      },
    });
...
export default async function Home() {
  ...
  return (
    <main className="min-h-screen bg-white pt-20 font-bold">
      {/* Hero Section */}
      <section className="py-2 px-4 text-center border-b border-gray-100 bg-gray-50/50">
      ...
      <section className="py-20 border-t border-gray-100 text-center">
        <p className="text-gray-300 text-[10px] tracking-[0.2em] uppercase">
          © 2025 Your Gallery. All rights reserved.
        </p>
      </section>
    </main>
  );
